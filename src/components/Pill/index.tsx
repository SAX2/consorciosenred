...
const Pill: React.FC<PillProps> = ({
  ...
  const variantContainerClassName = variant === "warning" ? "border-yellow-sec/15 bg-yellow/15" : variant === "error" ? "border-red/25 bg-red/15" : variant === "success" ? "border-green/15 bg-green/15" : variant === "neutral" ? "border-outline bg-grey-sec" : variant === "default" ? "border-outline bg-white" : "" 
  const variantTextClassName = variant === "warning" ? "text-yellow-sec" : variant === "error" ? "text-red" : variant === "success" ? "text-green" : variant === "neutral" ? "text-text-grey" : variant === "default" ? "text-black" : ""
  ...
  return (
    ...
    >
      {iconOrientation === "left" && icon}
      <span
        className={cn("dark:text-white font-medium", classNameText, variantTextClassName)}
      >
        {text}
      </span>
      {iconOrientation === "right" && icon}
