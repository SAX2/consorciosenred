...
const UnitGrid = ({ data }: { data: any }) => {
  ...
  useEffect(() => {
    const checkWidth = () => {
      if (divRef.current) {
        setIsSmall(divRef.current.offsetWidth < 740);
      }
    };
  ...
  return (
    ...
    >
      ...
      <div className={cn("border border-outline dark:border-outline-dark border-dashed rounded-lg flex items-center justify-center max-md:border-0")}>
        <NewUnitKeyDialog />
      </div>
    </div>
  );
